...
uniform vec4 bottom_tint : hint_color;

// Adapted from Gonkee, https://www.youtube.com/c/Gonkee
...
void fragment() {
	...
	float bottom = (UV.y - 0.15 + distort_sum.y) / (2.8 / sprite_scale.y);
	bottom = clamp(bottom, 0.0, 3.0);
	
	color = mix(color, bottom_tint, bottom);
